...
def _step(p, rps, p50, p95, p100, qmax, err):
    return {
        "parallelism": p, "ok": 20 - err, "error_count": err, "skipped": 0,
        "throughput_rps": rps, "queue_ms": {"p50": 0, "p100": qmax},
        "latency_ms": {"p50": p50, "p95": p95, "p99": p100, "p100": p100},
        "usage": {"reasoning": 1800}, "cost": {"total": 0.006},
    }


# A fake /sweep response — a curve that clearly degrades past P=25.
MOCK_SWEEP = {
    "n": 20, "p_list": [1, 5, 10, 25, 50], "total_cost": 0.031, "cost_cap": 10.0,
    "run_id": "665f1c2a9b3e4d0011aa9999", "persist_error": None,
    "steps": [
